...
function FileTree({ tree, projectRoot, projectName, onOpen, setTree, onOpenProject, refreshTree, closeTab, contextPaths, setContextPaths, gitStatus }: {
  tree: TreeNode[];
  ...
  const askName = (title: string, defaultValue = ""): Promise<string | null> => {
    return new Promise(resolve => {
      setPromptConfig({
        title,
        defaultValue,
        onConfirm: val => { setPromptConfig(null); resolve(val); },
    ...
    if (node.type === "folder") {
      return (
        <div key={node.id}>
          ...
          >
            <span className="tree-arrow">{node.open ? "��" : "��"}</span>
          ...
          {node.open && node.children?.map(c => renderNode(c, depth + 1, currentPath))}
        </div>
      );
    }
    return (
