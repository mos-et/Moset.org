...
export function Explorador({ tree, projectRoot, projectName, onOpen, setTree, onOpenProject, refreshTree, closeTab, contextPaths, setContextPaths, gitStatus, floatButton }: {
  tree: TreeNode[];
      ...
      return (
        <div key={node.id}>
          <div
            ...
            onDragStart={(e) => { 
              e.stopPropagation(); 
              e.dataTransfer.setData("application/moset-path", fullPath); 
              e.dataTransfer.setData("text/plain", `[Carpeta: ${node.name}](${fullPath})`);
            }}
          ...
          >
            ...
            <button 
              ...
              onClick={(e) => {
                e.stopPropagation();
                if (inContext) {
                   setContextPaths?.(prev => prev.filter(p => p !== fullPath));
                } else {
                   setContextPaths?.(prev => [...prev, fullPath]);
    ...
    return (
      <div
        ...
        onDragStart={(e) => { 
          e.stopPropagation(); 
          e.dataTransfer.setData("application/moset-path", fullPath); 
          // Agregar texto plano para permitir soltar en textareas y Monaco Editor
          e.dataTransfer.setData("text/plain", `[${node.name}](${fullPath})`);
        }}
        ...
        <button 
          ...
          onClick={(e) => {
            ...
            if (inContext) {
               ...
               setContextPaths?.(prev => [...prev, fullPath]);
               // Auto-activar el modo contexto si estaba apagado
               if (localStorage.getItem("moset_context_mode") !== "selected") {
                 localStorage.setItem("moset_context_mode", "selected");
                 window.dispatchEvent(new Event("moset-settings-updated"));
               }
  ...
  return (
    <div className="file-tree">
      <div className="sidebar-section-title" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>EXPLORADOR{floatButton}</div>
