...
export function ActivityBar({ openPanels, togglePanel, chatOpen, setChatOpen }: ActivityBarProps) {
  const items = [
    { id: "explorer", icon: (
    ...
    { id: "gguf", icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
        ...
        <line x1="9.5" y1="13.5" x2="10.2" y2="14.2"/>
        <line x1="13.8" y1="15.8" x2="14.5" y2="16.5"/>
      </svg>
    ), label: "GGUF Config" },
    { id: "uast", icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div className="activity-bar">
      ...
      {items.map(item => (
        <button
          key={item.id}
          className={`activity-btn ${openPanels.has(item.id) ? "active" : ""}`}
